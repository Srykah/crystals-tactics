...
Container::Container(char scrollDirection)
: mChildren()
, mSelectedChild(-1)
...
bool Container::handleEvent(const Input::Event& event)
{
	...
	else //else navigate through the container
    {
        ...
        if(hasSelection())
        {
            if (event.stdAc == Input::Up && (mScrollDirection & VERTICAL))
                selectPrevious();
            else if (event.stdAc == Input::Left && (mScrollDirection & HORIZONTAL))
                selectPrevious();
            else if (event.stdAc == Input::Down && (mScrollDirection & VERTICAL))
                selectNext();
            else if (event.stdAc == Input::Right && (mScrollDirection & HORIZONTAL))
                selectNext();
