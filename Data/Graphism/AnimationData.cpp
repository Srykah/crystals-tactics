...
AnimationData::AnimationData(const sf::Texture& texture, sf::Time duration, bool repeat)
: texture(texture)
, duration(duration)
, repeat(repeat)
{
...
void AnimationData::loadFromNode(const pugi::xml_node& node)
{
...
std::size_t AnimationData::getNumFrames() const
{
	return frames.size();
}
